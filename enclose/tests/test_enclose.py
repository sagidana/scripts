...
def enclose(*argv, stdin=''):
    result = subprocess.run([sys.executable, '-m', 'enclose'] + list(argv),
                            cwd=FIXTURES,
                            input=stdin,
                            capture_output=True,
                            text=True,
...
def test_range_output_is_accepted_as_input():
    assert enclose('--range', stdin='sample.py:14:16\n') == 'sample.py:14:16\n'


def test_line_without_column():
    assert enclose('--range', stdin='sample.py:15:some text\n') == 'sample.py:14:16\n'


def test_class():
    assert enclose('--class', '--range', stdin='sample.py:15:21:x\n') == 'sample.py:8:22\n'


def test_inner_includes_lambda():
    hit = 'sample.py:27:26:    callback = lambda x: helper(x)\n'
    assert enclose('--range', stdin=hit) == 'sample.py:25:29\n'
    assert enclose('--range', '--inner', stdin=hit) == 'sample.py:27:27\n'


def test_outer_picks_outermost():
    hit = 'sample.py:21:20:            return n - 1\n'
    assert enclose('--range', stdin=hit) == 'sample.py:20:21\n'
    assert enclose('--range', '--outer', stdin=hit) == 'sample.py:18:22\n'


def test_callers():
    out = enclose('--range', '--callers', '1', stdin='sample.py:4:5:def helper(value):\n')
...
def test_callees():
    out = enclose('--range', '--callees', '2', stdin='sample.c:19:1:x\n')
...
def test_root_limits_the_index(tmp_path):
    out = enclose('--range', '--callers', '1', '--root', str(tmp_path), stdin='sample.py:4:5:x\n')
    assert out == 'sample.py:4:5\n'


def test_lines_as_arguments_and_dash_for_stdin():
    out = enclose('--range', 'sample.go:14:11:x', '-', stdin='sample.rs:11:21:x\n')
...
def test_file_of_hits_as_argument(tmp_path):
    ...
    assert enclose('--range', str(hits)) == 'sample.el:5:6\n'


def test_shebang_picks_the_grammar():
    assert enclose('--range', stdin='run:4:5:x\n') == 'run:3:4\n'


def test_lang_forces_the_grammar():
    assert enclose('--range', '--lang', 'python', stdin='notes.txt:1:1:x\n') == 'notes.txt:1:1:x\n'


def test_script_inside_html():
    assert enclose('--range', stdin='sample.html:9:10:x\n') == 'sample.html:8:10\n'
